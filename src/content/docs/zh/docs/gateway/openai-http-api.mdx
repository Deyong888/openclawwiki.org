---
title: OpenAI 聊天补全 (HTTP)
description: 从 Gateway 暴露一个 OpenAI 兼容的 /v1/chat/completions HTTP 端点。
sidebar:
  label: OpenAI API
  order: 22
---
...
OpenClaw 的 Gateway 可以服务于一个小型 OpenAI 兼容的聊天补全端点。
...
如果请求包含 OpenAI `user` 字符串，Gateway 会从中派生一个稳定的会话密钥，以便重复调用可以共享代理会话。
