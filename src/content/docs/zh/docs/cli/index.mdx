---
summary: "`openclaw` 命令、子命令和选项的 CLI 参考"
read_when:
  - 添加或修改 CLI 命令或选项
  - 记录新的命令界面
title: "CLI 参考"
...
- OSC-8 超链接在支持的终端中渲染为可点击链接；否则回退到普通 URL。
- `--json` (以及支持的地方 `--plain`) 禁用样式以获得干净的输出。
- `--no-color` 禁用 ANSI 样式；`NO_COLOR=1` 也被遵守。
...
调色板真理来源: `src/terminal/palette.ts` (又名 “lobster seam”)。
...
- `openclaw security audit` — 审计配置 + 本地状态以查找常见的安全隐患。
- `openclaw security audit --deep` — 尽力而为的实时 Gateway 探测。
- `openclaw security audit --fix` — 收紧安全默认值并 chmod state/config。
...
大多数插件更改需要 gateway 重启。参见 [/plugin](/plugin)。
...
- `/status` 快速诊断。
- `/config` 持久化配置更改。
- `/debug` 运行时配置覆盖 (仅内存，不存盘; 需要 `commands.debug: true`)。
...
- `--wizard`: 运行引导向导。
- `--non-interactive`: 运行无提示向导。
- `--mode <local|remote>`: 向导模式。
- `--remote-url <url>`: 远程 Gateway URL。
- `--remote-token <token>`: 远程 Gateway token。

当存在任何向导标志时 (`--non-interactive`, `--mode`, `--remote-url`, `--remote-token`)，向导自动运行。

### `onboard`

交互式向导，用于设置 gateway, workspace, 和 skills。

选项：

- `--workspace <dir>`
- `--reset` (在向导前重置 config + credentials + sessions + workspace)
- `--non-interactive`
- `--mode <local|remote>`
...
非交互式配置助手 (get/set/unset)。运行不带子命令的 `openclaw config` 会启动向导。
...
健康检查 + 快速修复 (config + gateway + 旧版服务)。

选项：

- `--no-workspace-suggestions`: 禁用工作区内存提示。
- `--yes`: 接受默认值而不提示 (headless)。
...
- `channels list`: 显示配置的频道和认证 profile。
- `channels status`: 检查 gateway 可达性和频道健康状况 (`--probe` 运行额外检查；使用 `openclaw health` 或 `openclaw status --deep` 进行 gateway 健康探测)。
- 提示: `channels status` 在检测到常见错误配置时会打印带有建议修复的警告 (然后指向 `openclaw doctor`)。
- `channels logs`: 显示来自 gateway 日志文件的最近频道日志。
- `channels add`: 未传递标志时为向导式设置；传递标志时切换到非交互模式。
...
- `--no-usage`: 跳过模型提供商用量/配额快照 (仅限 OAuth/API-backed)。
- `--json`: 输出 JSON (包含用量，除非设置了 `--no-usage`)。
...
- `skills info <name>`: 显示单个技能的详情。
- `skills check`: 就绪 vs 缺失需求的摘要。
...
- `webhooks gmail run` (用于相同标志的运行时覆盖)
...
添加一个新的隔离 agent。除非传递标志 (或 `--non-interactive`)，否则运行引导向导；非交互模式下 `--workspace` 是必填的。
...
OpenClaw 可以在 OAuth/API 凭据可用时展示提供商用量/配额。

展示位置：

- `/status` (可用时添加一行简短的提供商用量)
...
- 数据直接来自提供商用量端点 (无估算)。
- 提供商：Anthropic, GitHub Copilot, OpenAI Codex OAuth, 加上 Gemini CLI/Antigravity (当这些插件启用时)。
- 如果不存在匹配的凭据，用量将被隐藏。
...
- `gateway status` 默认使用服务的解析端口/配置探测 Gateway RPC (可用 `--url/--token/--password` 覆盖)。
- `gateway status` 支持 `--no-probe`, `--deep`, 和 `--json` 用于脚本。
- `gateway status` 还能检测旧版或额外的 gateway 服务 (`--deep` 增加系统级扫描)。以 Profile 命名的 OpenClaw 服务被视为一等公民，不会被标记为“额外”。
- `gateway status` 打印 CLI 使用的配置路径 vs 服务可能使用的配置 (服务 env)，加上解析的探测目标 URL。
- `gateway install|uninstall|start|stop|restart` 支持 `--json` 用于脚本 (默认输出保持人类友好)。
- `gateway install` 默认为 Node 运行时；bun **不推荐** (WhatsApp/Telegram bug)。
...
Gateway CLI 助手 (使用 `--url`, `--token`, `--password`, `--timeout`, `--expect-final` 用于 RPC 子命令)。
...
提示：当直接调用 `config.set`/`config.apply`/`config.patch` 时，如果配置已存在，传递来自 `config.get` 的 `baseHash`。
...
总是包含认证存储中 profile 的认证概览和 OAuth 过期状态。
`--probe` 运行实时请求 (可能会消耗 token 并触发速率限制)。
...
- `cron add` (别名: `create`; 需要 `--name` 和恰好一个 `--at` | `--every` | `--cron`, 以及恰好一个负载 `--system-event` | `--message`)
- `cron edit <id>` (修补字段)
...
`node` 运行一个 **无头节点主机** 或作为后台服务管理它。参见 [`openclaw node`](/cli/node)。
...
`nodes` 与 Gateway 对话并目标配对的节点。参见 [/nodes](/nodes)。
