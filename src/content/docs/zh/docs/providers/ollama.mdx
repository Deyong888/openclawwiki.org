...
3. 为 OpenClaw 启用 Ollama（任何值都可以；Ollama 不需要真正的密钥）：
...
确保 Ollama 正在运行，并且您设置了 `OLLAMA_API_KEY`（或身份验证配置文件），并且您 **未** 定义显式的 `models.providers.ollama` 条目：
...
- [模型提供商](/zh/docs/concepts/model-providers) - 所有提供商概览
- [模型选择](/zh/docs/concepts/models) - 如何选择模型
- [配置](/zh/docs/gateway/configuration) - 完整配置参考
