...
description: Nextcloud Talk 支持状态、功能和配置
sidebar:
...
状态：通过插件支持（Webhook 机器人）。支持私信、房间、回应 (reactions) 和 markdown 消息。
...
如果在配置/引导期间选择 Nextcloud Talk 并且检测到 git 检出，OpenClaw 将自动提供本地安装路径。
...
## 注意事项
- 机器人无法发起私信。用户必须先给机器人发消息。
- Webhook URL 必须可被网关访问；如果位于代理后面，请设置 `webhookPublicUrl`。
...
- 公共私信: `channels.nextcloud-talk.dmPolicy="open"` 加上 `channels.nextcloud-talk.allowFrom=["*"]`.
...
```
- 要允许无房间，保持白名单为空或设置 `channels.nextcloud-talk.groupPolicy="disabled"`.
...
- `channels.nextcloud-talk.chunkMode`: `length` (默认) 或 `newline` 在长度分块之前先在空行（段落边界）拆分。
