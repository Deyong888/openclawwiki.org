---
title: Agent Send CLI
description: 直接运行 `openclaw agent` CLI（带可选投递）。
sidebar:
  label: Agent Send CLI
  order: 20
---

# `openclaw agent` (直接代理运行)

`openclaw agent` 运行单个代理回合，无需入站聊天消息。
默认情况下，它**通过网关**；添加 `--local` 以强制在当前机器上运行嵌入式运行时。

## 行为

- 必需：`--message <text>`
- 会话选择：
  - `--to <dest>` 派生会话密钥（组/频道目标保留隔离；直接聊天折叠为 `main`），**或**
...
- 输出：
  ...
  - `--json`：打印结构化负载 + 元数据
- 使用 `--deliver` + `--channel` 可选地投递回频道（目标格式匹配 `openclaw message --target`）。
