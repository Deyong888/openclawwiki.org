---
title: 浏览器故障排除 (Linux)
description: 修复 OpenClaw 浏览器控制在 Linux 上的 Chrome/Brave/Edge/Chromium CDP 启动问题。
sidebar:
  label: Linux 故障排除
  order: 31
---
...
OpenClaw 的浏览器控制服务器无法启动 Chrome/Brave/Edge/Chromium，错误如下：
```
...
在 Ubuntu（以及许多 Linux 发行版）上，默认的 Chromium 安装是 **snap 包**。Snap 的 AppArmor 限制干扰了 OpenClaw 启动和监控浏览器进程的方式。
...
安装官方的 Google Chrome `.deb` 包，它没有被 snap 沙盒化：
...
| `browser.executablePath` | 基于 Chromium 的浏览器二进制文件路径 (Chrome/Brave/Edge/Chromium) | 自动检测（当为 Chromium 时首选默认浏览器） |
...
注意：
- `chrome` 配置文件尽可能使用你的 **系统默认 Chromium 浏览器**。
