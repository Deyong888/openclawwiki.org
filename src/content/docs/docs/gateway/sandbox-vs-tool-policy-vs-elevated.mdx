...
See [Sandboxing](/docs/gateway/sandboxing) for the full matrix (scope, workspace mounts, images).
...
- `/exec` is separate from elevated. It only adjusts per-session exec defaults for authorized senders. See [Elevated Mode](/docs/tools/elevated).
