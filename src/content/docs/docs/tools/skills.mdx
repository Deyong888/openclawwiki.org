...
entry. See [Plugins](/docs/plugins) for discovery/config and [Tools](/docs/tools) for the
...
- Prefer sandboxed runs for untrusted inputs and risky tools. See [Sandboxing](/docs/gateway/sandboxing).
...
See [Skills config](/docs/tools/skills-config) for the full configuration schema.
