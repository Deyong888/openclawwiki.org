---
title: Tools
description: Agent tool surface for OpenClaw (browser, canvas, nodes, message, cron).
sidebar:
  label: Overview
  order: 0
---
...
See [Plugins](/docs/plugins) for install + config, and [Skills](/docs/tools/skills) for how
...
- [LLM Task](./llm-task): JSON-only LLM step for structured workflow output (optional schema validation).
...
- gateway/node approvals and allowlists: [Exec approvals](/docs/tools/exec-approvals).
...
- See [Web tools](/docs/tools/web) for setup.
- See [Firecrawl](/docs/tools/firecrawl) for the optional anti-bot fallback.
