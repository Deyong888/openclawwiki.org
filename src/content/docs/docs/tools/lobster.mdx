...
See [LLM Task](/docs/tools/llm-task) for details and configuration options.
...
OpenProse pairs well with Lobster: use `/prose` to orchestrate multi-agent prep, then run a Lobster pipeline for deterministic approvals. If a Prose program needs Lobster, allow the `lobster` tool for sub-agents via `tools.subagents.tools`. See [OpenProse](/docs/prose).
...
- [Plugins](/docs/plugin)
- [Plugin tool authoring](/docs/plugins/agent-tools)
